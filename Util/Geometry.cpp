...
//			texture.id = LoadDDSTexture(device, commandList, L".\\Textures\\stone.dds", m_descriptorHeap.get(), &m_stoneTexture[0]); 
...
GeometryGenerator::MeshData GeometryGenerator::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	Material mat;
	// process material
	//OutputDebugStringA(string(mesh->mMaterialIndex));
	//OutputDebugStringA("mesh->MaterialIndex: ");
	//print(mesh->mMaterialIndex);
...
void GeometryGenerator::processNode(aiNode* node, const aiScene* scene)
{
