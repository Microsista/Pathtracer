#include "AccelerationStructureComponent.h"

inline AccelerationStructureBuffers AccelerationStructureComponent::BuildBottomLevelAS(const std::vector<D3D12_RAYTRACING_GEOMETRY_DESC>& geometryDescs, D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS buildFlags) {
...
inline AccelerationStructureBuffers AccelerationStructureComponent::BuildTopLevelAS(AccelerationStructureBuffers bottomLevelAS[BottomLevelASType::Count], D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS buildFlags)
{
    auto device = deviceResources->GetD3DDevice();
    auto commandList = deviceResources->GetCommandList();
    Microsoft::WRL::ComPtr<ID3D12Resource> scratch;
...
inline void AccelerationStructureComponent::BuildAccelerationStructures() {
    auto device = deviceResources->GetD3DDevice();
    auto commandList = deviceResources->GetCommandList();
    ...
    LoadDDSTexture(device, commandList, s.c_str(), descriptorHeap.get(), &stoneTexture[0].resource, &stoneTexture[0].upload, &stoneTexture[0].heapIndex, &stoneTexture[0].cpuDescriptorHandle, &stoneTexture[0].gpuDescriptorHandle, D3D12_SRV_DIMENSION_TEXTURE2D);
    ...
    for (int i = 0; i < materials.size(); i++) {
        ...
        print(path);
        if (add != "")
            LoadDDSTexture(device, commandList, wstring(path.begin(), path.end()).c_str(), descriptorHeap.get(), &templeTextures[i]);
        ...
        print(path2);
        if (normalMapPath != "")
            LoadDDSTexture(device, commandList, wstring(path2.begin(), path2.end()).c_str(), descriptorHeap.get(), &templeNormalTextures[i]);
        ...
        print(path3);
        if (specularMapPath != "")
            LoadDDSTexture(device, commandList, wstring(path3.begin(), path3.end()).c_str(), descriptorHeap.get(), &templeSpecularTextures[i]);
        ...
        print(path4);
        if (emittanceMapPath != "")
            LoadDDSTexture(device, commandList, wstring(path4.begin(), path4.end()).c_str(), descriptorHeap.get(), &templeEmittanceTextures[i]);
    }
...
inline void AccelerationStructureComponent::BuildGeometryDescsForBottomLevelAS(std::array<std::vector<D3D12_RAYTRACING_GEOMETRY_DESC>, BottomLevelASType::Count>& geometryDescs) {
...
inline void AccelerationStructureComponent::BuildBottomLevelASInstanceDescs(BLASPtrType* bottomLevelASaddresses, Microsoft::WRL::ComPtr<ID3D12Resource>* instanceDescsResource) {
    ...
    vector<InstanceDescType> instanceDescs(NUBLAS);
